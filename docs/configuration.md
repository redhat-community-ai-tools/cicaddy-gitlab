...
Set variables in **Project → Settings → CI/CD → Variables**. Mark secrets as **Masked**, check **Expand variable reference**, and leave **Protect variable** unchecked for variables used in MR pipelines.
...
| "GitLab API authentication failed" / `401 Unauthorized` | Create a Project Access Token with `api` scope and store as `GITLAB_TOKEN` CI/CD variable with **Expand variable reference** checked and **Protect variable** unchecked (see [Getting Started — GitLab API Token](getting-started.md#gitlab-api-token)) |
| `Permission denied on resource project $GOOGLE_CLOUD_PROJECT` (literal `$`) | CI/CD variable not reaching the job — check **Expand variable reference** is checked and **Protect variable** is unchecked |
